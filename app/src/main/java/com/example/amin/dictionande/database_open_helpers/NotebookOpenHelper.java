    ...
    public List<Notebook> getNotebookList() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                VocabularyOpenHelper vocabularyOpenHelper = new VocabularyOpenHelper(context,notebook.getNoteBookName());

                notebook.setWordsCount(vocabularyOpenHelper.getRawsCount());
                notebook.setBookmarkedCount(vocabularyOpenHelper.getBookmarkRawsCount());
...
//        }
    ...
    public void returnNotebook(Notebook notebook) {
        ...
        // این قسمت از کد برای تست اضافه شده است{
            if(getRawsCount() == 0){
                return;
            }
        //{
...
//        sqLiteDatabase.update(NOTEBOOK_TABLE_NAME, cv, COL_ID + " = ?", new String[]{String.valueOf(notebook.getId())});
    ...
    public Notebook getRandomIsPlayingNotebook(){
        ...
        for (int i = 0; i < notebooks.size(); i++) {
            if(new VocabularyOpenHelper(context,notebooks.get(i).getNoteBookName()).getRawsCount()>0){
        ...
        return valuableNotebooks.get(RandomPlayingVoc.randInt(0,valuableNotebooks.size()-1));
    }
