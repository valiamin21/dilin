...
public class ShowWordsWidgetOpenHelper extends SQLiteOpenHelper {
    ...
    public Voc getPlayVoc() {
        ...
        return
                new VocabularyOpenHelper(context, getCurrentNotebookName())
                        .getVoc(vocId);
    }
...
//    }
    ...
    private int getNextWordId() {
        ...
        int nextWordId = new VocabularyOpenHelper(context, getCurrentNotebookName()).getNextId(currentWordID);
...
//                putCursorOnContentValues(cursor,cv);
    ...
    public void deletedSomeNotebook() {
        // TODO: 1/31/19 این تابع باید زمانی فراخوانی شود که یک دفتر به طور کامل حذف می شود. اگر این دفتر در لیست پخش هم بود باید حذف شود و اگر به عنوان دفتر در حال پخش بود باید ابتدا به دفتر بعدی رفت و سپس این دفتر را حذف کرد.
    }
